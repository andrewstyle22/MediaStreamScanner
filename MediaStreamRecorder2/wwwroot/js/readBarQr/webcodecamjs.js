...
var WebCodeCamJS = function(element) {
    ...
    //var mediaDevices = window.navigator.mediaDevices.getUserMedia;
    //navigator.mediaDevices.getUserMedia(constraintsInicio);// esto tengo que corregirlo para android
    ...
    var display = Q(element),
        ...
        options = {
            ...
            zoom: 0,
            beep: 'audio/beep.mp3',
            decoderWorker: 'js/readBarQr/DecoderWorker.js',
            brightness: 0,
    ...
    function initBarCodeQr() {
        ...
        try {
            ...
            console.log("Catch of initBarCodeQr");
    ...
    function play() {
        clearResultImageAndPhoto();
        document.getElementById('play').disabled = true;
        document.getElementById('stop').disabled = false;

        //Rec button
        document.getElementsByClassName('j-start')[0].disabled = true;
        //document.querySelectorAll('.j-start')[0].disabled = true;
        if (!localImage) {
            if (!localStream) {
                initBarCodeQr();
            }
    ...
    function stop() {
        ...
        //Rec button
        document.getElementsByClassName('j-start')[0].disabled = false;
        //document.querySelectorAll('.j-start')[0].disabled = true;
        delayBool = true;
        const p = video.pause();
        if (p && (typeof Promise !== 'undefined') && (p instanceof Promise)) {
                ...
                if (e.data.success === true && e.data.success != 'localization') {
                    ...
                    setTimeout(function() {
                        if (options.codeRepetition || lastCode != e.data.result[0].Value) {
                            beep();
                            localImage = true; // this way the application stops        //Rec button
                            document.getElementsByClassName('j-start')[0].disabled  = false;
                            document.getElementById('play').disabled = false;
    ...
    function clearResultImageAndPhoto() {
        document.getElementById("canvas").style.display = 'none'; // hide photo
        document.getElementById("canvas").style.visibility = 'hidden'; // hide photo

        document.getElementById("videoRecord").style.display = 'none'; // hide photo
        document.getElementById("videoRecord").style.visibility = 'hidden'; // hide photo

        document.getElementById("scanned-img").style.display = 'block'; // hide img scanner
        document.getElementById("scanned-img").style.visibility = 'visible'; // hide img scanner
